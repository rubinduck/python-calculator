...
def run_test_app(MainWindowClass, *args, widget_args=(), keyword_widget_args={}):
    ...
    window = MainWindowClass(
        *args, widget_args=widget_args, keyword_widget_args=keyword_widget_args)
    window.show()
    app.exec()
...
def test_GeneralControlButtonsPanel():
    ...
    run_test_app(Main, (200, 200, 200, 200), GeneralControlButtonsPanel,
                 keyword_widget_args={"row_length": 2})


def test_MainControlButtonsWidget():
    MainControlButtonsWidget = gui.MainControlButtonsWidget
    run_test_app(WidgetTestingContainer, (200, 200, 200, 200),
                 MainControlButtonsWidget)


def test_CalculatorMainWindowGui():
